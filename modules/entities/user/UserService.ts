...
class UserService {
    ...
    authorize = async (login: string, password: string): Promise<{ user: IUser | null, error: string | null }> => {
        try {
            const users = require('../../../__mocks__/users.json');
            const result = this.processingResponse(users, login, password);
            if (!result) {
                return { user: null, error: 'Incorrect login or code' };
            } else {
                return { user: result, error: null };
            }
        } catch (error) {
            console.warn('UserService -> authorize: ', error);
            return { user: null, error: 'Request error' };
        }
    }

    private processingResponse = (users: { data: IDefaultUser[] }, login: string, password: string): IUser | null => {
